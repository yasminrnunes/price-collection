"""Scraping script for Extra marketplace using GPA Digital API.

This module provides functionality to scrape product data from Extra.com.br
...
Stats:
    - Products loaded:  13583
    - Time spent:       19,42 minutes
...
API Details:
    - Endpoint: https://api.vendas.gpa.digital/ex/search/category-page
    - Method: POST
    - Authentication: None required (public API)
    - Store ID: 483 (Extra store)
    - Results per page: 100

Example:
    Run the scraper:
        >>> python market_extra_api.py
...
def _create_request_body(category: str, page: int = 1) -> Dict[str, Any]:
    """Create API request body for Extra category search.
    ...
    Note:
        The request includes fixed parameters like partner ("linx"), storeId (483),
        and department ("ecom") that are specific to the Extra marketplace.
    """
    return json.dumps(
        {
            "partner": "linx",
...
def _make_api_request(category: str, page: int = 1) -> Optional[Dict[str, Any]]:
    """Make API request to Extra/GPA Digital API with error handling.
    
    This function makes a POST request to the Extra category search endpoint
...
def _extract_product_data(
    ...
    This function extracts product information from the Extra API response and
    creates a ScrapingProduct object with all associated promotions and discounts.
...
def main():
    """Main function to run the Extra marketplace scraper.
